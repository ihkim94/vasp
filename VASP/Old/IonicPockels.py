...
def susceptibility_derivatives(disp=0.1, natoms=45):
    d_chi_d_tau = []
    ...
    return np.array(d_chi_d_tau)


def calculate_Raman(d_chi_d_tau, phonon):
    phonon = phonon.flatten()
    return np.einsum('ijk,i->jk', d_chi_d_tau, phonon)
...
if __name__ == "__main__":
    ...
    born = get_Born(vasprun_born="../04-Dielectric/vasprun.xml")
    d_chi_d_tau = susceptibility_derivatives(disp=0.1, natoms=5)

    for band in np.arange(4, 16):
        
        freq, eig = get_phonons(qpoint_file="../03-Phonons/qpoints_NAC.yaml", band=band, poscar="POSCAR")
